...
pub use bakeoff::{
    BAKEOFF_CONTRACT_JSON, BAKEOFF_MANIFEST_SCHEMA, BAKEOFF_PUBLIC_REPORT_SCHEMA, BakeoffHashInput,
    ValidatedBakeoffManifest, load_and_validate_bakeoff_manifest,
    load_and_validate_bakeoff_manifest_for_measurement, validate_bakeoff_manifest_text,
    validate_bakeoff_manifest_text_for_measurement, validate_bakeoff_public_report_text,
...
USAGE:
    ...
    transcript-quality validate-bakeoff-report <report.json>
...
validate-bakeoff-report
                       Check a candidate public report against the frozen
                       schema: only declared fields, no forbidden private
                       fields, bound to the frozen contract and manifest hash
...
pub fn run(args: impl IntoIterator<Item = impl AsRef<str>>) -> Result<(), String> {
    ...
    match all.first().map(String::as_str) {
        ...
        Some("validate-bakeoff-report") => run_validate_bakeoff_report(all.into_iter().skip(1)),
        _ => run_manifest(all),
    }
}
...
fn run_validate_bakeoff_report(
    ...
    let [report] = args.as_slice() else {
        return Err(
            "USAGE:\n    transcript-quality validate-bakeoff-report <report.json>".to_owned(),
        );
    };
    let text = std::fs::read_to_string(report)
        .map_err(|err| format!("cannot read bakeoff public report {report}: {err}"))?;
    bakeoff::validate_bakeoff_public_report_text(&text)?;
    println!("bakeoff public report {report} matches the frozen schema");
    Ok(())
}
