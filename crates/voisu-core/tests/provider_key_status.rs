...
fn a_success_status_is_valid() {
    assert_eq!(
        ProviderKeyStatus::classify(200, false),
        ProviderKeyStatus::Valid
    );
    assert_eq!(
        ProviderKeyStatus::classify(204, false),
        ProviderKeyStatus::Valid
    );
...
fn unauthorized_and_forbidden_mean_the_key_is_invalid() {
    assert_eq!(
        ProviderKeyStatus::classify(401, false),
        ProviderKeyStatus::InvalidKey
    );
    assert_eq!(
        ProviderKeyStatus::classify(403, false),
        ProviderKeyStatus::InvalidKey
    );
    ...
    assert!(
        ProviderKeyStatus::InvalidKey
            .headline()
            .contains("voisu setup")
    );
    assert_eq!(
        ProviderKeyStatus::InvalidKey.readiness(),
...
fn a_429_with_retry_after_is_a_transient_rate_limit() {
    ...
    assert_eq!(
        ProviderKeyStatus::RateLimited.readiness(),
...
fn other_statuses_are_transient_unreachable() {
    ...
    assert_eq!(
        ProviderKeyStatus::Unreachable.readiness(),
        ReadinessStatus::Warn
    );
}
