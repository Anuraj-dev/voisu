...
async fn main() {
    ...
    let path = std::env::args()
        .nth(1)
        .expect("usage: deepgram_probe <file.raw>");
    ...
    let sender = tokio::spawn(async move {
        for (index, chunk) in pcm.chunks(2048).enumerate() {
            if let Err(error) = sink.send(Message::Binary(chunk.to_vec())).await {
                eprintln!(
                    "[{:>6.2}s] SEND ERROR at chunk {index}: {error}",
                    started.elapsed().as_secs_f32()
                );
                return sink;
            }
        ...
        eprintln!(
            "[{:>6.2}s] all audio sent, sending CloseStream",
            started.elapsed().as_secs_f32()
        );
        let _ = sink
    ...
    loop {
        match stream.next().await {
            ...
            Some(Ok(other)) => eprintln!(
                "[{:>6.2}s] OTHER {other:?}",
                started.elapsed().as_secs_f32()
            ),
            Some(Err(error)) => {
                eprintln!(
                    "[{:>6.2}s] WS ERROR {error}",
                    started.elapsed().as_secs_f32()
                );
                break;
            }
