...
- **09 (2026-07-20):** Packaging accounts live (HITL, GH #41): GPG apt-signing key `4149…5B28` (ed25519, passphrased, offline backup verified; public key at packaging/apt/voisu-archive-keyring.asc); FAS+COPR `anuraj-dev/voisu` (ID 246563, F43/44 x86_64, recreated lowercase — COPR names case-sensitive); AUR `anuraj-dev` with dedicated no-passphrase deploy key (SSH auth verified; voisu/voisu-bin free, claim = ticket 11's push); GH secrets AUR_SSH_PRIVATE_KEY/GPG_PRIVATE_KEY/GPG_PASSPHRASE/COPR_WEBHOOK_URL. Deviations: AUR 2FA doesn't exist upstream (proposed post-Atomic-Arch, not shipped — compensating controls + queued HITL); Cloudsmith token conditional on ticket 13's in-ticket choice; COPR auto-rebuild flag is per-package → ticket 12.
- **07 (2026-07-20):** `voisu setup` + keyring storage + doctor classification live (PR #62, 5 review rounds): injected-IO wizard with signal-safe hidden entry; secret-tool boundary instead of the `keyring` crate (duplicate-D-Bus-stack avoidance, reviewer-endorsed); bounded store retry + LOUD flock-serialized 0600 fallback (unavailable/locked/tool-missing distinguished); real plaintext→keyring migration, prune outcome keyed on target-line content (gone/survived/unverifiable, single source of truth in remove); env override authoritative even when malformed (doctor FAILs naming the variable); load never blocks startup. Deferred: wizard-scale keyring deadline. Suites 431/0. PHASE A COMPLETE.
- **06 (2026-07-19):** Keyring probe on the live fresh-login boot: Secret Service reachable AND unlocked at voisu.service start — `ksecretd` (PAM-launched, NOT kwalletd6) owns `org.freedesktop.secrets` 29 s before the daemon; pam_kwallet auto-unlock; 45–48 ms round trips from a systemd-run service-like context, zero prompts. Ticket 07 design: keyring primary + short bounded lazy retry (≈4.25 s budget), loud file fallback only after, distinguishing unowned vs locked. Asset: assets/06-keyring-service-probe.md; next-reboot ExecStartPre probe kit prepared (assets/06-keyring-probe/, HITL, non-gating).
