...
if [[ ${1:-} == -Qp && ${2:-} == --info ]]; then
    ...
    for path in /usr/ /usr/bin/ /usr/bin/voisu /usr/bin/voisu-daemon \
        /usr/bin/voisu-overlay /usr/lib/ /usr/lib/systemd/ \
...
if [[ ${1:-} == --user && ${2:-} == show ]]; then
    ...
    if [[ ${SYSTEMD_MODE:-good} == daemon-inactive && $unit == voisu.service \
        ...
        printf "%s\\n" "ActiveState=active" "SubState=running" "MainPID=$PPID"
    fi
